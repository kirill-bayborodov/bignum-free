...
Use fixed seed, thread count, data-count and iteration counts when a result will become a baseline. The following command is the reproducible C11 runner contract after `make build CONFIG=release`; it uses the downloaded flat distribution directly.
