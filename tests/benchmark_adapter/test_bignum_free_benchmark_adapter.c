/**
 * @file test_bignum_free_benchmark_adapter.c
 * @brief Deterministic unit tests for the bignum_free benchmark-core adapter.
 *
 * @details
 ...
 */
typedef enum {
    BIGNUM_FREE_ADAPTER_TEST_STATUS_SUCCESS = 0, /**< The test helper's assertions succeeded. */
    BIGNUM_FREE_ADAPTER_TEST_STATUS_FAILURE = 1 /**< An observed adapter outcome differed from its contract. */
} bignum_free_adapter_test_status_t;
 ...
 * @return A named bignum_free_adapter_test_status_t result.
 *
 * @details
 ...
 */
static bignum_free_adapter_test_status_t bignum_free_adapter_test_make_workload(
    benchmark_workload_t *workload)
{
    if (workload == NULL) {
        return BIGNUM_FREE_ADAPTER_TEST_STATUS_FAILURE;
    }
    *workload = (benchmark_workload_t){
        ...
        .operation_kind = "free", /**< Exercise the complete representation wipe. */
    ...
    return BIGNUM_FREE_ADAPTER_TEST_STATUS_SUCCESS;
}
 ...
 */
static bignum_free_adapter_test_status_t bignum_free_adapter_test_validation(void)
{
    benchmark_workload_t workload;
    benchmark_workload_t invalid_workload;

    if (bignum_free_adapter_test_make_workload(&workload) !=
        BIGNUM_FREE_ADAPTER_TEST_STATUS_SUCCESS) {
        return BIGNUM_FREE_ADAPTER_TEST_STATUS_FAILURE;
    }
    if (bignum_free_benchmark_validate_workload(&workload) !=
        BIGNUM_FREE_BENCHMARK_STATUS_SUCCESS) {
        return BIGNUM_FREE_ADAPTER_TEST_STATUS_FAILURE;
    }
    invalid_workload = workload;
    invalid_workload.operation_kind = "xor";
    if (bignum_free_benchmark_validate_workload(&invalid_workload) !=
        BIGNUM_FREE_BENCHMARK_STATUS_INVALID_PROFILE) {
        return BIGNUM_FREE_ADAPTER_TEST_STATUS_FAILURE;
    }
    if (bignum_free_benchmark_validate_workload(NULL) !=
        BIGNUM_FREE_BENCHMARK_STATUS_NULL_ARGUMENT) {
        return BIGNUM_FREE_ADAPTER_TEST_STATUS_FAILURE;
    }
    return BIGNUM_FREE_ADAPTER_TEST_STATUS_SUCCESS;
}
 ...
 */
static bignum_free_adapter_test_status_t bignum_free_adapter_test_callbacks(void)
{
    ...
    if (bignum_free_benchmark_adapter_init(NULL) !=
        BIGNUM_FREE_BENCHMARK_STATUS_NULL_ARGUMENT) {
        return BIGNUM_FREE_ADAPTER_TEST_STATUS_FAILURE;
    }
    if (bignum_free_benchmark_adapter_init(&adapter) !=
        BIGNUM_FREE_BENCHMARK_STATUS_SUCCESS || adapter.initialize == NULL ||
    ...
    return checksum == 0U
        ? BIGNUM_FREE_ADAPTER_TEST_STATUS_FAILURE
        : BIGNUM_FREE_ADAPTER_TEST_STATUS_SUCCESS;
}
...
int main(void)
{
    if (bignum_free_adapter_test_validation() !=
        BIGNUM_FREE_ADAPTER_TEST_STATUS_SUCCESS) {
        fputs("bignum_free benchmark adapter validation test failed\n", stderr);
        return 1;
    }
    if (bignum_free_adapter_test_callbacks() !=
        BIGNUM_FREE_ADAPTER_TEST_STATUS_SUCCESS) {
        fputs("bignum_free benchmark adapter callback test failed\n", stderr);
        return 1;
    }
    puts("bignum_free benchmark adapter tests: OK");
    return 0;
}
